  ...
  _exec (name, args) {
    ...
    return (new Promise((resolve, reject) => {
      if (task.current && task.singleTrigger && task.singleTrigger !== 'N') {
        ...
        console.log(`[node-swarm-kb] Tasks._exec: Stacking task ${task.name} callback to current execution`);
        ...
        console.log(`[node-swarm-kb] Tasks._exec: Stacking task ${task.name} callback to next execution`);
      } else {
        ...
        console.log(`[node-swarm-kb] Tasks._exec: Queuing task ${task.name} (queue=${task.queue.length})`);
      }
      if (!task.current) {
        this._run(task);
      }
