...
class Tasks {
  constructor (swarm, options) {
    ...
    this._swarm.messages.on('_task', (message, topics, from) =>
      ...
      ? this.doExec(message.task, message.args || [])
  ...
  **/
  ...
  async _exec (name, key, args, retry = false) {
    const nodeIp = this.ring.get(key);
    if (!nodeIp || this._swarm.isMe(nodeIp)) {
      return this.doExec(name, args);
    } else {
      debug(`Sending task ${name} to node ${nodeIp}`);
      try {
        return await this._swarm.messages.send(nodeIp, '_task', { task: name, args: args });
      } catch (error) {
        if (error.code === 'ECONNREFUSED' && !retry) {
          await this._swarm.getNodes();
          return this._exec(name, key, args, true);
        }
        return Promise.reject(Object.assign(error, { message: `Task ${name} execution failed on remote node ${nodeIp}: ${error.message}` }));
      }
    }
  }
  ...
  **/
  async exec (name) {
    ...
    if (!this.ring) {
      this._init();
      if (!this.ring) {
        throw new Error('Ring not ready');
      }
    }
    ...
    return this._exec(name, key, args);
  }
