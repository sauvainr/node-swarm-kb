...
* @param options object see the readme
*/
function Kubernetes (options) {
  ...
  this.kubeApi = K8s.api({
    endpoint: `https://${this.options.host}:${this.options.port}`,
    version: '/api/v1',
    auth: {
  ...
  const processWatchedMessage = (data) => {
    ...
    console.log(`[node-swarm-kb] kubernetes: ${data.type} message from ${endpoint && endpoint.metadata && endpoint.metadata.selfLink}`);
    ...
    .then((nodes) => {
      ...
      (error) => {
        if (error.code === 'ESOCKETTIMEDOUT') {
          ...
          console.error(`[node-swarm-kb] kubernetes watching pod lost: ${error}, retrying in 5 sec.`);
...
Kubernetes.prototype = {
  ...
  _error: function _error (error, data) {
    typeof this.options.onError === 'function' ? this.options.onError(error, data) : console.error(`[node-swarm-kb] kubernetes error. ${error}`, data);
  },
  /**
  * Make a request to Kubernetes API
  * @param path string path after the namespace
  * @param query object query parameters
  * @param body object body data
  * @param method ['GET','PUT','POST','DELETE','PATCH'] default GET or POST if body
  * @return promise
  */
  request: function request (path, query, body, method) {
    if ((!method || method === 'GET') && this.resourceVersions[path]) {
      Object.assign({[`/api/v1/${path}`]: this.resourceVersions[path]}, query);
    }

    console.log(`[node-swarm-kb] kubernetes request: ${path}`, query || body);
    return Rp({
      url: `https://${this.options.host}:${this.options.port}/api/v1/${path}`,
      headers: {
      ...
      if ((!method || method === 'GET') && result.metadata && result.metadata.selfLink && result.metadata.resourceVersion) {
        ...
        console.log(`[node-swarm-kb] kubernetes request ${result.metadata.selfLink} version updated to ${result.metadata.resourceVersion}`);
      }
      return result;
    });
  },
  nsRequest: function nsRequest (path, query, body, method) {
  ...
  watch: function watch (path, query, onMessage, onError) {
    path = `namespaces/${this.options.namespace}/${path}`;
    console.log(`[node-swarm-kb] kubernetes watching: ${path}`, query);
    ...
    this.kubeApi.watch(`/${path}?${queryString}`, (result) => {
      if (result.object && result.object.metadata && result.object.metadata.selfLink && result.object.metadata.resourceVersion) {
        ...
        console.log(`[node-swarm-kb] kubernetes watch ${result.object.metadata.selfLink} version updated to ${result.object.metadata.resourceVersion}`);
      }
      onMessage(result);
    }, onError, 300000);
  },
  _addNode: function _addNode (node) {
    ...
    console.log(`[node-swarm-kb] kubernetes: node added: ${node.ip} total: `, Object.keys(this.nodes));
    if (typeof this.options.onNodeAdded === 'function') {
      this.options.onNodeAdded(node);
    }
  },
  _removeNode: function _removeNode (ip) {
    ip = ip.ip || ip;
  ...
  _endpointsToNodes: function _endpointsToNodes (endpoint) {
  ...
  getNodes: function getNodes () {
    ...
    if (!this.options.namespace) {
      return Promise.reject('Missing namespace');
    }
