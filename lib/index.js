  ...
  return new Promise((resolve, reject) => {
    ...
    this.messages = new Messages(Object.assign(this.options.messages || {}, {
      onError: (error) =>
        module.exports._handlers.error.length
        ? module.exports._handlers.error.map((handler) => handler(error))
        : console.error('[node-swarm-kb] messages Error', error)
    }));
  ...
  **/
  send (node, topic, message) {
    ...
    if (!topic || !topic.length) {
      error = new Error('[node-swarm-kb] send Error: Missing topic');
    }
    if (!node) {
      error = new Error('[node-swarm-kb] send Error: Missing node');
    }
    if (error) {
    ...
    module.exports.messages.send(node.ip || node, topic, message);
  },
  /**
